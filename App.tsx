...
const AppContent: React.FC = () => {
  ...
  const handleLoginConfirm = async () => {
    const role = loginModal.role;
    if (!role) return;

    // Close modal immediately or let the global loader takeover
    setLoginModal(prev => ({ ...prev, isOpen: false }));

    try {
      setIsLoading(true); // Trigger global load on login action
  ...
  const renderDashboard = () => {
    ...
    switch (user.role) {
      case UserRole.PATIENT:
        return <PatientDashboard user={user} currentView={currentPage} onUserUpdate={handleUserUpdate} />;
      case UserRole.DOCTOR:
        return <DoctorDashboard user={user} currentView={currentPage} onUserUpdate={handleUserUpdate} />;
      case UserRole.RESEARCHER:
        return <ResearcherDashboard user={user} currentView={currentPage} onUserUpdate={handleUserUpdate} />;
      case UserRole.ADMIN:
        return <AdminDashboard user={user} currentView={currentPage} onUserUpdate={handleUserUpdate} />;
      default:
