...
import { Camera, Save, Mail, MapPin, Award, Loader2, ShieldCheck, Phone, Globe, Clock, Star, Edit3, Calendar, User as UserIcon, Activity } from 'lucide-react';
...
const DoctorProfile: React.FC<DoctorProfileProps> = ({ user, onUserUpdate }) => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const newAvatarUrl = await StorageService.uploadImage(file);
             ...
             <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-start">
                 ...
                 <div className="flex-shrink-0 relative mx-auto md:mx-0 group">
                     ...
                     <button 
                        ...
                        className="absolute -bottom-3 -right-3 p-3 bg-primary text-white rounded-2xl shadow-lg hover:bg-blue-600 transition-all border-4 border-white dark:border-slate-800 hover:scale-110 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
                     >
                         <Camera size={18} />
                     </button>
                 </div>
