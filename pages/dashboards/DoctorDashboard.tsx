...
import PatientUpdateModal from '../../components/doctor/PatientUpdateModal'; // Imported new modal
...
const DashboardHome: React.FC<{ user: User }> = ({ user }) => {
  ...
  // Live Clock & Overdue Check
  useEffect(() => {
    const timer = setInterval(() => setCurrentTime(new Date()), 60000);
    
    // Check for overdue patient updates (Notification Logic)
    const checkOverdueUpdates = async () => {
        const patients = await MockDatabase.getPatientsForDoctor(user.id);
        const sevenDaysAgo = Date.now() - (7 * 24 * 60 * 60 * 1000); // 7 Days threshold
        
        const overduePatients = patients.filter(p => {
             const lastUpdate = new Date(p.lastUpdated).getTime();
        ...
        if (overduePatients.length > 0) {
             setAlerts(prev => {
                ...
                if (prev.some(a => a.msg === 'Update Pending')) return prev;
                
                return [{
                    id: Date.now(),
                    type: 'warning',
                    msg: 'Update Pending',
                    sub: `${overduePatients.length} patient records need updates.`
                }, ...prev];
             });
        }
    };
    ...
    >
      <PatientUpdateModal 
         ...
         onUpdateComplete={() => {
             ...
             setAlerts(prev => prev.filter(a => a.msg !== 'Update Pending'));
      ...
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
           <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">System Live</span>
           </div>
           <h1 className="text-4xl font-display font-bold text-slate-900 dark:text-white tracking-tight">
             Good Morning, <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">{user.name.split(' ')[1]}</span>
           </h1>
           <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">
             You have <span className="text-slate-900 dark:text-white font-bold">{alerts.length} urgent alerts</span> and <span className="text-slate-900 dark:text-white font-bold">8 appointments</span> today.
           </p>
        </div>
      ...
      <motion.div variants={containerVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        {[
          { label: 'Appointments', value: 8, icon: Calendar, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/20', trend: '+2' },
          ...
          { label: 'Total Patients', value: 1240, icon: Users, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-900/20', trend: '+15%' },
              ...
              <table className="w-full">
                  <thead>
                      <tr className="border-b border-slate-100 dark:border-slate-800 text-left">
                          ...
                          <th className="py-4 px-4 text-xs font-bold uppercase text-slate-400 tracking-wider">Diagnosis</th>
                  ...
                  <tbody>
                      {[
                          { name: 'Sarah Khan', diag: 'Migraine', date: 'Today, 10:30 AM', status: 'In-Progress', img: 'https://picsum.photos/seed/sarah/200' },
                          { name: 'Rahul Sharma', diag: 'Type 2 Diabetes', date: 'Yesterday', status: 'Completed', img: 'https://picsum.photos/seed/rahul/200' },
                          { name: 'Amit Verma', diag: 'Hypertension', date: 'Oct 24, 2023', status: 'Critical', img: 'https://picsum.photos/seed/amit/200' },
                      ].map((row, i) => (
                          <motion.tr 
                             key={i} 
                          ...
                          >
                              <td className="py-4 px-4">
                                  <div className="flex items-center gap-3">
                                      <div className="relative">
                                        <img src={row.img} className="w-10 h-10 rounded-full bg-slate-200 object-cover" alt={row.name} />
                                        {row.status === 'Critical' && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 border-2 border-white rounded-full"></span>}
                                      </div>
                              ...
                              <td className="py-4 px-4">
                                  <span className={`text-xs font-bold px-3 py-1 rounded-full border ${
                                      row.status === 'Critical' ? 'bg-red-50 text-red-600 border-red-100 animate-pulse' : 
                                      row.status === 'In-Progress' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'
                                  }`}>
                                      {row.status}
                                  </span>
                              </td>
...
const DoctorDashboard: React.FC<DoctorDashboardProps> = ({ user, currentView, onUserUpdate }) => {
  const renderContent = () => {
    switch(currentView) {
      case 'profile':
        return <DoctorProfile user={user} onUserUpdate={onUserUpdate} />;
      case 'patients':
