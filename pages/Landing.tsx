...
import { 
  ArrowRight, Activity, Sparkles, Database,
...
const AnimatedIcon: React.FC<{ icon: React.ElementType, size?: number, className?: string }> = ({ icon: Icon, size = 24, className }) => (
  <motion.div
    variants={iconReveal}
    className={className}
  >
...
const SpotlightCard: React.FC<{
  ...
  glowColor?: string;
  delay?: number;
}> = ({ children, onClick, className = "", glowColor = "rgba(0, 122, 255, 0.4)", delay = 0 }) => {
  ...
  return (
    <motion.div
      ...
      variants={fadeUpItem}
      onClick={onClick}
      whileHover={{ y: -8, scale: 1.01, transition: HOVER_SPRING }}
      className={`glass-panel-apple rounded-[2.5rem] relative overflow-hidden group cursor-pointer will-change-transform ${className}`}
    >
      {/* Reactive Border */}
      <div 
        ref={borderRef}
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-24">
      <motion.div 
        ...
        variants={fadeUpItem}
        className="glass-panel-apple rounded-[2.5rem] p-10 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden border border-white/60 dark:border-white/20 shadow-2xl"
      >
        ...
        <div className="flex items-start gap-6 z-10 relative">
          <motion.div 
            variants={iconReveal}
            className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary to-blue-600 flex items-center justify-center text-white shadow-neon-blue ring-4 ring-white/10"
          >
             <Database size={32} />
          </motion.div>
          <div>
        ...
        <div className="flex items-center gap-8 z-10">
           ...
           <Link to="/research">
             <motion.button 
               ...
               whileTap={{ scale: 0.95 }}
               className="btn-futuristic px-10 py-5 rounded-full font-bold text-sm flex items-center gap-3 overflow-hidden group transition-all"
             >
...
const RoleCard: React.FC<{
  ...
  <motion.button 
    variants={fadeUpItem}
    onClick={onClick}
    whileHover={{ y: -10, transition: HOVER_SPRING }}
    whileTap={{ scale: 0.98 }}
    className="group relative flex flex-col items-start text-left p-8 rounded-[2.5rem] glass-panel-apple w-full h-full border-t border-white/50 dark:border-white/10 hover:border-primary/50 transition-colors will-change-transform"
  >
    ...
    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-all duration-500 ease-out group-hover:scale-110 group-hover:rotate-3 shadow-lg ${colorClass.includes('teal') ? 'bg-teal-50 dark:bg-teal-500/20 text-teal-600 dark:text-teal-400' : colorClass.includes('blue') ? 'bg-blue-50 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400' : colorClass.includes('purple') ? 'bg-purple-50 dark:bg-purple-500/20 text-purple-600 dark:text-purple-400' : 'bg-orange-50 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400'}`}>
      <AnimatedIcon icon={Icon} size={32} />
    </div>
...
const Landing: React.FC<LandingProps> = ({ onLoginClick }) => {
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden font-sans text-slate-900 dark:text-dark-text bg-transparent selection:bg-primary/30">
      ...
      <motion.header 
        style={{ y: y1, opacity }}
        className="relative z-10 min-h-[90vh] flex flex-col justify-center items-center px-6 pt-24 perspective-1000 will-change-transform"
      >
        ...
        {/* Hero Content */}
        <div className="text-center max-w-6xl mx-auto mb-12 relative">
           ...
           <motion.div 
             ...
             className="inline-flex items-center space-x-3 glass-panel-apple rounded-full pl-2 pr-6 py-2 mb-10 transition-all hover:scale-105 mx-auto border border-white/50 dark:border-white/20 shadow-lg hover:shadow-neon-teal cursor-default"
           >
           ...
           <motion.h1 
             ...
             className="text-7xl md:text-9xl font-display font-bold mb-8 tracking-tighter leading-[0.95] drop-shadow-2xl"
           >
               ...
               <span className="block text-transparent bg-clip-text bg-gradient-to-r from-primary via-blue-400 to-secondary pb-4 relative animate-gradient-x">
                 ...
                 >
                    <Sparkles className="text-secondary w-16 h-16 animate-pulse-slow opacity-80 filter drop-shadow-[0_0_10px_rgba(52,199,89,0.5)]" />
                 </motion.div>
               </span>
           </motion.h1>
           ...
           <motion.p 
             ...
             className="text-xl md:text-3xl text-slate-700 dark:text-slate-200 mb-14 leading-relaxed max-w-3xl mx-auto font-light drop-shadow-md"
           >
           ...
           >
                <motion.button 
                  ...
                  className="btn-futuristic px-12 py-6 rounded-full font-bold text-base md:text-lg flex items-center justify-center gap-3 tracking-widest uppercase group transition-all"
                >
                ...
                <motion.button 
                  ...
                  className="btn-futuristic-secondary px-12 py-6 rounded-full font-bold text-base md:text-lg flex items-center justify-center gap-3 tracking-widest uppercase group shadow-lg hover:shadow-xl transition-all"
                >
      ...
      <section className="py-32 px-6 relative z-10">
        <div className="max-w-7xl mx-auto">
          <motion.div 
            ...
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16 h-auto lg:h-[650px]"
          >
            
            {/* Main Feature - Large */}
            <SpotlightCard className="md:col-span-2 lg:col-span-2 h-full p-10 md:p-12 flex flex-col justify-between group bg-white/40 dark:bg-white/5" glowColor="rgba(0, 122, 255, 0.5)">
            ...
            <div className="flex flex-col gap-8 h-full">
                 <SpotlightCard className="h-full p-8 md:p-10 flex flex-col justify-center" glowColor="rgba(52, 199, 89, 0.4)">
                    <div className="w-14 h-14 bg-green-50 dark:bg-green-900/30 rounded-2xl flex items-center justify-center text-secondary mb-6 shadow-sm ring-1 ring-green-500/20">
                       <AnimatedIcon icon={ShieldCheck} size={28} />
                    </div>
                 ...
                 <SpotlightCard className="h-full p-8 md:p-10 flex flex-col justify-center" glowColor="rgba(255, 149, 0, 0.4)">
                    <div className="w-14 h-14 bg-orange-50 dark:bg-orange-900/30 rounded-2xl flex items-center justify-center text-orange-500 mb-6 shadow-sm ring-1 ring-orange-500/20">
                       <AnimatedIcon icon={UploadCloud} size={28} />
                    </div>
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto">
         ...
         <motion.div 
           ...
           whileInView="visible"
           viewport={{ once: true, margin: "-100px" }}
           className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 h-auto lg:h-[420px]"
         >
           {[
             ...
             { role: UserRole.ADMIN, icon: Zap, title: 'Admin', desc: 'Manage facility operations and user compliance.', color: 'from-orange-400/30 to-orange-600/30' },
           ].map((item) => (
               <RoleCard 
                 key={item.role}
                 role={item.role}
