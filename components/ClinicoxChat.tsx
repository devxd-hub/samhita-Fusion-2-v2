...
import { MessageSquare, X, Send, Sparkles, Bot, Minimize2, Loader2, Zap, Mic, MicOff } from 'lucide-react';
...
const ClinicoxChat: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (!apiKey) {
        console.warn("GEMINI_API_KEY is not set. Chat AI features will be unavailable.");
        return;
      }
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-slate-50/50 dark:bg-black/20">
           {messages.map((msg, idx) => (
             ...
             >
                ...
                <div 
                  className={`max-w-[80%] p-3.5 rounded-2xl text-sm leading-relaxed shadow-sm relative group transition-all duration-300 ${
                    msg.role === 'user' 
