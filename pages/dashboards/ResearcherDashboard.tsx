...
import { 
  ...
  Tag, Activity, Loader2, Filter, ChevronDown, X, Eye, SlidersHorizontal,
  GraduationCap, BookOpen, Globe, Link as LinkIcon, Save, Camera,
  MapPin, Award, ShieldCheck, Microscope, FlaskConical, Beaker,
  PieChart, TrendingUp, Fingerprint, Star, Trash2, Plus, Check, ShieldAlert, Merge, Wand2, Users, Database,
...
const INITIAL_DB = [
  { id: 1, name: "Rahul Sharma", diagnosis: "Hypertension", organSystem: "Cardiology", diseaseType: "Lifestyle-linked", severity: "Moderate", stage: "Chronic", age: 45, gender: "Male" },
  { id: 2, name: "Rahul S.", diagnosis: "High BP", organSystem: "Cardiology", diseaseType: "Lifestyle-linked", severity: "Moderate", stage: "Chronic", age: 46, gender: "Male" }, // Duplicate candidate
  { id: 3, name: "Anita Roy", diagnosis: "Migraine", organSystem: "Neurology", diseaseType: "Chronic", severity: "Mild", stage: "Early-stage", age: 32, gender: "Female" },
  { id: 4, name: "Suresh P.", diagnosis: "Osteoarthritis", organSystem: "Unknown", diseaseType: "Degenerative", severity: "Severe", stage: "Late-stage", age: 68, gender: "Male" }, // Missing Dept
  { id: 5, name: "Amit Verma", diagnosis: "Arrhythmia", organSystem: "Unknown", diseaseType: "Cardiovascular", severity: "Critical", stage: "Acute", age: 50, gender: "Male" }, // Missing Dept
  { id: 6, name: "Baby A.", diagnosis: "Neonatal Jaundice", organSystem: "Geriatrics", diseaseType: "Metabolic", severity: "Mild", stage: "Early-stage", age: 0, gender: "Male" }, // Consistency Error (Baby in Geriatrics)
  { id: 7, name: "John Doe", diagnosis: "Pregnancy Checkup", organSystem: "Obstetrics & Gynecology", diseaseType: "N/A", severity: "Normal", stage: "Mid-stage", age: 30, gender: "Male" }, // Consistency Error (Male in OBGYN)
  { id: 8, name: "Suresh Patel", diagnosis: "Joint Pain", organSystem: "Orthopedics", diseaseType: "Degenerative", severity: "Severe", stage: "Late-stage", age: 68, gender: "Male" }, // Duplicate candidate for #4
  { id: 9, name: "Maria Garcia", diagnosis: "Type 2 Diabetes", organSystem: "Endocrinology", diseaseType: "Metabolic", severity: "Moderate", stage: "Chronic", age: 55, gender: "Female" },
  { id: 10, name: "Chen Wei", diagnosis: "Glaucoma", organSystem: "Ophthalmology", diseaseType: "Degenerative", severity: "Severe", stage: "Late-stage", age: 72, gender: "Male" },
  { id: 11, name: "Fatima Al-Sayed", diagnosis: "Asthma", organSystem: "Pulmonology", diseaseType: "Inflammatory", severity: "Mild", stage: "Chronic", age: 24, gender: "Female" },
  { id: 12, name: "Unknown Patient", diagnosis: "Fracture", organSystem: "Unknown", diseaseType: "Trauma-related", severity: "Severe", stage: "Acute", age: 30, gender: "Male" },
];

// --- Sub Components ---

const ResearcherProfile: React.FC<{ user: User; onUserUpdate: (user: User) => void }> = ({ user, onUserUpdate }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    hIndex: '42',
    citations: '3,450',
    grantsAmount: 'Active: ₹12.5 Cr',
    interests: 'Genomics, Prakriti, AI Diagnostics, Epigenetics',
    website: 'https://scholar.google.com/vikram-singh'
  });

  const [collaborators, setCollaborators] = useState([
    { id: 1, name: 'Dr. Sarah Lee', role: 'Post-Doc', bg: 'bg-blue-100 text-blue-600' },
    { id: 2, name: 'Arjun Mehta', role: 'PhD Scholar', bg: 'bg-orange-100 text-orange-600' },
    { id: 3, name: 'Priya K.', role: 'Research Asst', bg: 'bg-pink-100 text-pink-600' },
  ]);

  const [grants, setGrants] = useState([
    { id: 1, code: 'NIH-RO1-2023', status: 'Active', name: 'AyurGenomics Study' },
    { id: 2, code: 'Ayush-CoE-Grant', status: 'Review', name: 'Center of Excellence' }
  ]);
  ...
  const handleSave = async () => {
    ...
    // Simulate DB update
    await MockDatabase.updateDoctorProfile(user.id, { name: formData.name, email: formData.email, avatar: avatar });
    // Update global state
    onUserUpdate({ ...user, name: formData.name, email: formData.email, avatar: avatar });
    setIsSaving(false);
    setSuccess(true);
  ...
  const handleAddCollaborator = () => {
    if (!newCollab.name.trim() || !newCollab.role.trim()) return;
    
    const colors = ['bg-blue-100 text-blue-600', 'bg-orange-100 text-orange-600', 'bg-pink-100 text-pink-600', 'bg-purple-100 text-purple-600', 'bg-teal-100 text-teal-600'];
  ...
  const toggleGrantStatus = (id: number) => {
    setGrants(prev => prev.map(g => {
        if(g.id === id) {
            const nextStatus = g.status === 'Active' ? 'Review' : g.status === 'Review' ? 'Pending' : 'Active';
      ...
      <div className="relative max-w-6xl mx-auto px-4 -mt-20 z-10">
         ...
         <div className="glass-panel-apple bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-[2.5rem] p-6 md:p-8 shadow-2xl border border-white/50 dark:border-white/10">
             <div className="flex flex-col md:flex-row gap-8 items-start">
                 <div className="flex-shrink-0 relative mx-auto md:mx-0 group">
                     <div className="w-32 h-32 md:w-40 md:h-40 rounded-[2rem] overflow-hidden border-4 border-white dark:border-slate-800 shadow-2xl relative bg-slate-100">
                         <img src={avatar} alt={formData.name} className="w-full h-full object-cover transition-transform group-hover:scale-105" />
                         {isUploadingImage && (
                     ...
                     <button 
                        ...
                        className="absolute -bottom-3 -right-3 p-3 bg-purple-600 text-white rounded-2xl shadow-lg hover:bg-purple-700 transition-all border-4 border-white dark:border-slate-800 hover:scale-110 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
                        title="Upload Photo"
                     >
                         <Camera size={18} />
                     </button>
                 ...
                 <div className="flex-1 w-full pt-2 text-center md:text-left">
                     <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
                         ...
                         <div className="flex gap-3 w-full lg:w-auto justify-center">
                             ...
                             >
                                {isSaving ? <Loader2 className="animate-spin" size={20} /> : success ? <Check size={20} /> : <Save size={20} />}
                                {isSaving ? 'Updating...' : success ? 'Saved!' : 'Save Profile'}
                             </button>
                         </div>
                     </div>
                 </div>
             ...
             <div className="space-y-8">
                      ...
                      <div className="relative z-10">
                          ...
                          <div className="mb-6">
                              <div className="flex justify-between items-end mb-2">
                                  ...
                                  <span className="text-2xl font-bold text-emerald-400">{formData.grantsAmount}</span>
                              </div>
                          ...
                          <div className="space-y-3">
                              {grants.map(grant => (
                                <div 
                                    key={grant.id} 
                                    onClick={() => toggleGrantStatus(grant.id)}
                                    className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 cursor-pointer transition-colors"
                                >
                                    <div>
                                        <span className="text-xs font-bold block">{grant.code}</span>
                                        <span className="text-[10px] text-slate-400 block">{grant.name}</span>
                                    </div>
                                    <span className={`text-xs px-2 py-1 rounded font-bold ${
                                        grant.status === 'Active' ? 'text-emerald-400 bg-emerald-500/10' : 
                                        grant.status === 'Review' ? 'text-amber-400 bg-amber-500/10' :
                          ...
                          <p className="text-[10px] text-slate-500 text-center mt-3">Click grant to update status</p>
                      </div>
                 </div>
                 ...
                 <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-sm border border-slate-100 dark:border-slate-800">
                     <div className="flex items-center justify-between mb-6">
                         ...
                         <span className="text-xs text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg">{collaborators.length} Members</span>
                     </div>
                     ...
                     <div className="space-y-4">
                         <AnimatePresence>
                             {collaborators.map((member) => (
                                 <motion.div 
                                    ...
                                    className="flex items-center gap-4 p-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group"
                                 >
                                     <div className={`w-10 h-10 rounded-full ${member.bg} flex items-center justify-center font-bold text-sm shrink-0`}>
                                     ...
                                     <button 
                                        ...
                                        className="ml-auto p-2 rounded-full text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-all"
                                     >
                                         <Trash2 size={16} />
                                     </button>
                                 </motion.div>
                         ...
                         {isAddingCollab ? (
                             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 bg-slate-50 dark:bg-black/20 rounded-2xl space-y-3 border border-slate-200 dark:border-slate-700">
                                 <input 
                                    type="text" 
                                    placeholder="Name (e.g. Dr. Jane)" 
                                    className="w-full bg-white dark:bg-slate-800 p-2 rounded-lg text-sm border border-slate-200 dark:border-slate-700 outline-none"
                                 ...
                                 <div className="flex gap-2">
                                     ...
                                     <button 
                                        ...
                                        className="flex-1 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold py-2 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600"
                                     >
                                         Cancel
                                     </button>
                                 </div>
                             ...
                             <button 
                                ...
                                className="w-full py-3 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 text-xs font-bold text-slate-500 hover:text-purple-500 hover:border-purple-500 transition-colors flex items-center justify-center gap-2"
                             >
              ...
              <div className="relative z-10">
                  ...
                  <button 
                     ...
                     className={`w-full py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50 ${
                         consistency.status === 'complete' && consistency.issues.length > 0
                   ...
                   <div key={category} className="relative">
                      <button 
                         ...
                         className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider border transition-all flex items-center gap-2 ${
                            activeFilters.some(f => FILTER_CATEGORIES[category].includes(f))
                      ...
                      <tr key={record.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                         ...
                         <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded-md text-xs font-bold border ${
                               record.organSystem === 'Unknown' ? 'bg-slate-100 text-slate-500 border-slate-200' : 
                         ...
                         <td className="px-6 py-4">
                            <div className="flex items-center gap-2">
                               <div className={`w-2 h-2 rounded-full ${
                                  record.severity === 'Critical' ? 'bg-red-500 animate-pulse' : 
...
const ResearcherDashboard: React.FC<ResearcherDashboardProps> = ({ user, currentView, onUserUpdate }) => {
  const renderContent = () => {
    switch(currentView) {
      case 'profile':
        return <ResearcherProfile user={user} onUserUpdate={onUserUpdate} />;
      case 'analytics':
